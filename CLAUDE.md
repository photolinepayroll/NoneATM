...
|---|---|---|---|
| Standalone (primary, actively maintained) | `index.html`, `admin.html` (repo root) | GitHub Pages, auto-deployed on every push to `main` | `index.html`'s form submit: `fetch()` POST (`doPost` in `Code.gs`). `admin.html`'s data calls (`listSubmissions`/`getSubmissionsFields`/`getSubmissionsMedia`/`validatePasscode`): JSONP via a dynamically-created `<script src="...&callback=...">` tag (`doGet` in `Code.gs`) — switched from `fetch()` POST because `fetch()` was intermittently receiving an HTML error page instead of JSON from the exec URL's redirect hop (`script.google.com` → `script.googleusercontent.com`) in some network/browser environments; `<script>`-tag loading follows that same redirect at the browser's native script-loading layer and doesn't have the same failure mode. Both use the hardcoded `APPS_SCRIPT_URL` constant near the top of each file's `<script>`. Trade-off: the admin passcode now travels as a URL query parameter for these calls (visible in browser history/server logs) instead of a POST body — still fully gated by `requireAdmin_` server-side either way, nothing is readable without it. |
...
- Exposes the same admin actions (`listSubmissions`, `getSubmissionsFields`, `getSubmissionsMedia`, `validatePasscode`) via both `doPost` (JSON body) and `doGet` (JSONP, `?action=...&callback=...`) — `doGet` dispatches through `handleAdminJsonp_()` to the identical underlying functions, so there's one implementation per action, not two. `getSubmissionDetail` remains `doPost`/`google.script.run`-only (legacy `apps-script/Admin.html` surface). Plus the implicit form-submit path (`doPost` only, unchanged — large file uploads don't fit a GET query string).
